...
export const CheckoutPage: React.FC = () => {
  ...
  const [confirmedOrderData, setConfirmedOrderData] = useState<{
    ...
    items: { name: string; quantity: number; unit: string; subtotal: number }[];
    total: number;
    notes: string;
  } | null>(null);
  ...
  const handleSubmitOrder = async () => {
    ...
    try {
      ...
      // Save order data for WhatsApp message before clearing cart
      setConfirmedOrderData({
        clientName: user ? (user.name || '') : guestName,
        clientPhone: user ? (user.phone || '') : guestPhone,
        clientAddress: user ? (user.address || '') : guestAddress,
        paymentMethod: paymentMethod || '',
        items: items.map(item => ({
          name: item.product.name,
          quantity: item.quantity,
          unit: item.product.unit,
          subtotal: (item.product.final_price || item.product.price) * item.quantity
        })),
        total: total,
        notes: notes
      });
  ...
  const handleSendWhatsApp = () => {
    ...
    const items = confirmedOrderData.items.map(item => 
      `- ${item.name}: ${item.quantity} ${item.unit} - ${formatPrice(item.subtotal)}`
    ).join('\n');
    
    const message = `*Nuevo Pedido*\n\n` +
      `*Cliente:* ${confirmedOrderData.clientName}\n` +
      (confirmedOrderData.clientPhone ? `*Teléfono:* ${confirmedOrderData.clientPhone}\n` : '') +
      (confirmedOrderData.clientAddress ? `*Dirección:* ${confirmedOrderData.clientAddress}\n` : '') +
      (confirmedOrderData.paymentMethod ? `*Forma de Pago:* ${paymentMethodLabels[confirmedOrderData.paymentMethod] || confirmedOrderData.paymentMethod}\n` : '') +
      `\n*Productos:*\n${items}\n\n` +
      `*Total:* ${formatPrice(confirmedOrderData.total)}\n` +
      (confirmedOrderData.notes ? `\n*Notas:* ${confirmedOrderData.notes}` : '');
      ...
      <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
        <Card className="max-w-md w-full text-center">
          <CardContent className="pt-8 pb-8">
            ...
            <p className="text-gray-500 mb-6">Envía tu pedido por WhatsApp para confirmar:</p>
            <div className="space-y-3">
              <Button
                className="w-full text-xl py-6 bg-green-500 hover:bg-green-600 flex items-center justify-center gap-2"
